...
describe('Ao chamar o controller de getAll', () => {
  describe('quando não existem filmes no banco de dados', async () => {
    ...
    after(() => { MoviesService.getAll.restore(); })
  ...
  describe('quando existem filmes no banco de dados', async () => {
    ...
    before(() => {
      ...
      sinon.stub(MoviesService, 'getAll').resolves([ { id: '604cb554311d68f491ba5781', title: 'Example Movie', directedBy: 'Jane Dow', releaseYear: 1999, } ]);
    })
    after(() => { MoviesService.getAll.restore(); })
...
describe('Ao chamar o controller de create', () => {
  ...
  describe('quando é inserido com sucesso', async () => {
    ...
    before(() => {
      request.body = { title: 'Example Movie', directedBy: 'Jane Dow', releaseYear: 1999, };
      response.status = sinon.stub().returns(response);
      response.send = sinon.stub().returns();
